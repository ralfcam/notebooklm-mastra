...
const DEFAULT_VOICES = {
  ...
  sarah: "s3://voice-cloning-zero-shot/e040bd1b-f190-4bdb-83f0-75ef85b18f84/original/manifest.json",
};

export const generatePodcastAudio = createTool({
  id: "generate-podcast-audio",
  description: "Generate podcast audio from a transcript using Play AI's text-to-speech API",
  ...
  execute: async ({ context }) => {
    ...
    if (!response.ok) {
      throw new Error(`Failed to start audio generation: ${response.statusText}`);
    }
